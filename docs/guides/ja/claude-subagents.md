...
| ディスパッチ | `spec-quick` が固定フローで各フェーズの Subagent を呼び出す | `/kiro-impl` の自律モードがタスクごとに実装者+レビューアを起動 |
...
1. **タスク選択**: `tasks.md` から未完了の次タスクを1つ選択する
...
4. **タスク完了**: `tasks.md` の進捗を更新し、次のタスクへ（1タスク1イテレーション）
