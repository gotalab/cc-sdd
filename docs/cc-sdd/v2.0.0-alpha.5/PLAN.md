...
## 概要
`/kiro:spec-tasks <feature-name>` 実行時にデフォルトで並列判定を行い、並列実行可能なタスクへ `(P)` マーカーを付与した tasks.md を生成（必要に応じて `--sequential` でオプトアウト可能）
...
**変更内容**:
- `argument-hint` をデフォルト並列モード・オプトアウト引数（`--sequential`）に合わせて更新
...
**追加セクション**:
- デフォルト並列モードの追加制約（`--sequential` 指定時にのみ無効化）
- 並列マーカー `(P)` の付与ルール
- 並列タスクの依存関係検証ルール
...
3. **コマンド更新**
   ...
   - 並列分析ルールを標準ロードし、`--sequential` 指定時のみ無効化

4. **検証**
   - サンプルスペックでフラグなし（デフォルト並列）と `--sequential` 指定時の両ケースをテスト
