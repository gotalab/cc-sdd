...
- If `REQUIRES_HUMAN: false` → discard the failed implementation (`git checkout .`), then spawn a **new** implementer subagent with the debug report's `FIX_PLAN` and `NOTES` included in the prompt alongside the standard task context
