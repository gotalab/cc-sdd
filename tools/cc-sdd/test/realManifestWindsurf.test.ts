...
describe('real windsurf manifest', () => {
  ...
  it('shows windsurf recommendation message after applying plan', async () => {
    ...
    expect(out).toContain('npx cc-sdd@latest --devin');
    expect(out).toContain('Launch Windsurf / Cascade (deprecated) and run `/kiro-spec-init <what-to-build>` to create a new specification.');
